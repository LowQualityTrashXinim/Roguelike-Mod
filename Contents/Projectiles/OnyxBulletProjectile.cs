...
public class OnyxBulletProjectile : ModProjectile {
	...
	public override string Texture => ModUtils.GetVanillaTexture<Projectile>(ProjectileID.BulletHighVelocity); 
	...
	public override bool PreDraw(ref Color lightColor) {
		...
		default(LaserQuad).Draw(Projectile.Center,Projectile.velocity.ToRotation(), new Vector2(Projectile.velocity.Length() * LaserLength,  100 * (Projectile.timeLeft / 15f)), shaderSettings);

		shaderSettings.image1 = TextureAssets.Extra[193];
		shaderSettings.image2 = null;
		shaderSettings.image3 = null;
		shaderSettings.Color = Color.DarkMagenta;
		shaderSettings.shaderData = new Vector4((float)Projectile.timeLeft / 10, ModUtils.InOutSine((float)Projectile.timeLeft / 10),0, Projectile.ai[2]);
