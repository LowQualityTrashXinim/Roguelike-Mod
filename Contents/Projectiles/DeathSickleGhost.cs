...
public class DeathSickleGhost : ModProjectile {
	...
	public override bool PreDraw(ref Color lightColor) {
		...
		if (currentState == State.dashing || currentState == State.readjust) {
			...
			gts.image1 = ModContent.Request<Texture2D>(ModTexture.Perlinnoise);
			gts.Color = Color.Purple;
			gts.offset = Vector2.Zero;
			gts.shaderType = "TrailEffect";
			gts.shaderData = new Vector4(0, 0, 0, 0);
			gts.image2 = ModContent.Request<Texture2D>(ModTexture.Perlinnoise);
	...
	public override void AI() {
		...
		switch(currentState) 
		{
			...
			case State.idle:
				...
				dashTarget = Projectile.Center.LookForHostileNPCPositionClosest(2000, false);
...
public class DeathSickleClone : ModProjectile 
{
	...
	public override string Texture => ModUtils.GetVanillaTexture<Projectile>(ProjectileID.DeathSickle);
	public override void SetDefaults() {
		Projectile.CloneDefaults(ProjectileID.DeathSickle);
