...
namespace Roguelike.Contents.NPCs {
	internal class LootBoxLord : ModNPC {
		...
		private void ShootGun(Player player) {
			if (++gunCounter >= 30) {
				gunCounter = 0;
				int boomStick = ModUtils.NewHostileProjectile(NPC.GetSource_FromAI(), NPC.Center, (NPC.Center - player.Center).SafeNormalize(Vector2.Zero) * 20, ModContent.ProjectileType<HostileBoomStick>(), NPC.damage, 2, NPC.target);
			...
			if (BossDelayAttack(ModUtils.ToSecond(5), 0, 0)) {
				return;
			}
			int direction;
	...
	class OreBowAttackTwo : BaseHostileBow {
		public override void AI() {
			...
			if (++Projectile.ai[0] >= 30) {
				ModUtils.NewHostileProjectile(Projectile.GetSource_FromAI(), Projectile.Center, Vector2.UnitY * 15, ProjectileID.WoodenArrowHostile, Projectile.damage, 1, AdjustHostileProjectileDamage: false);
				Projectile.ai[0] = 0;
			}
		}
	}
