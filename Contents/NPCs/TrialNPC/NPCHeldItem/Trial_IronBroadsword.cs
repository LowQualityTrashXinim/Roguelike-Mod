...
namespace Roguelike.Contents.NPCs.TrialNPC.NPCHeldItem;
internal class Trial_IronBroadsword : ModProjectile {
	public override string Texture => ModUtils.GetVanillaTexture<Item>(ItemID.IronBroadsword);
	...
	public override void AI() {
		...
		percentDone = Math.Clamp(ModUtils.InOutExpo(percentDone, 11f), 0, 1);
	...
	public override void ModifyDamageHitbox(ref Rectangle hitbox) {
		...
		ModUtils.ModifyProjectileDamageHitbox(ref hitbox, npc.Center, outrotation, Projectile.width, Projectile.height, 10);
	}
	public override bool PreDraw(ref Color lightColor) {
		Main.instance.LoadProjectile(Projectile.type);
		Texture2D texture = ModContent.Request<Texture2D>(ModUtils.GetVanillaTexture<Item>(ItemID.IronBroadsword)).Value;
