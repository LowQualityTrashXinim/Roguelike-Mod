...
public class EnergyDrawPanel : Roguelike_UIPanel {
	public override void PreDraw(SpriteBatch spriteBatch) {
		...
		ModUtils.DrawProgressLine(spriteBatch,
...
public partial class TransmutationUIState {
...
public partial class TransmutationUIState : UIState {
...
public partial class TransmutationUIState {
	public void ItemShiftInit() {
		...
		btn_ItemShiftConfirm.SetPostTex(ModContent.Request<Texture2D>(ModUtils.GetTheSameTextureAsEntity<TransmuteTablet>()));
...
public partial class TransmutationUIState {
	...
	private void EnergyItemslot_OnLeftClick(UIMouseEvent evt, UIElement listeningElement) {
		...
		if (item.IsAWeapon() || item.accessory || item.IsThisArmorPiece() || item.type == ModContent.ItemType<Relic>() || item.type == 0) {
			if (listeningElement.UniqueId == energyItemslot1.UniqueId) {
				ModUtils.SimpleItemMouseExchange(player, ref energyItemslot1.item);
			}
			else if (listeningElement.UniqueId == energyItemslot2.UniqueId) {
				ModUtils.SimpleItemMouseExchange(player, ref energyItemslot2.item);
			}
			else if (listeningElement.UniqueId == energyItemslot3.UniqueId) {
				ModUtils.SimpleItemMouseExchange(player, ref energyItemslot3.item);
			}
			else if (listeningElement.UniqueId == energyItemslot4.UniqueId) {
				ModUtils.SimpleItemMouseExchange(player, ref energyItemslot4.item);
			}
		}
	}
	...
	private void Btn_confirm_OnLeftClick(UIMouseEvent evt, UIElement listeningElement) {
		if (Relicslot1.item.type != 0 && Relicslot2.item.type != 0 && Relicresultslot.item.type == 0) {
			if (Relicslot1.item.ModItem is Relic relic1 && Relicslot2.item.ModItem is Relic relic2) {
				...
				var handler = Main.LocalPlayer.ModPlayerStats();
	...
	private void Slot_OnLeftClick(UIMouseEvent evt, UIElement listeningElement) {
		...
		if (listeningElement.UniqueId == Relicslot1.UniqueId) {
			ModUtils.SimpleItemMouseExchange(player, ref Relicslot1.item);
		}
		else if (listeningElement.UniqueId == Relicslot2.UniqueId) {
			ModUtils.SimpleItemMouseExchange(player, ref Relicslot2.item);
		}
	}
	...
	public static int GetItemRarityDB(int rare, int option) {
		switch (option) {
			case 1:
				return ModItemLib.Safe_GetWeaponRarity(rare);
			case 2:
				return ModItemLib.Safe_GetAccRarity(rare);
			case 3:
				return ModItemLib.Safe_GetHeadRarity(rare);
			case 4:
				return ModItemLib.Safe_GetBodyRarity(rare);
			case 5:
				return ModItemLib.Safe_GetLegsRarity(rare);
			default:
				return ItemID.None;
		}
	}
