...
namespace Roguelike.Contents.Items.NoneSynergy.FairFrozen;
...
public class FairFrozenMinion : ModProjectile {
	...
	public override void AI() {
		var dustEff = Dust.NewDustDirect(Projectile.Center, 0, 0, DustID.Frost);
		...
		if (npc == null) {
			if (Projectile.Center.LookForHostileNPC(out NPC target, 1500)) {
				...
				var ToPos = player.Center.Add((Projectile.width * .25f + 5) * (index + 1) * player.direction, 0);
			...
			if (AICooldown <= 0) {
				...
				if (!Projectile.Center.IsCloseToPosition(npc.Center, 150 + offsetsize)) {
					...
					for (int i = 0; i < 30; i++) {
						var posToSpawm = Main.rand.NextVector2CircularEdge(12.5f, 4.5f).RotatedBy(Projectile.velocity.ToRotation() + MathHelper.PiOver2) * 2;
						var dust = Dust.NewDustDirect(Projectile.Center.PositionOFFSET(Projectile.velocity, -30), 0, 0, DustID.Frost);
						dust.noGravity = true;
