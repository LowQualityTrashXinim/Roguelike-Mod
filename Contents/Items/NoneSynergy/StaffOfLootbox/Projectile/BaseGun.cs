...
public class LBL_Minishark : BaseGun {
		...
		if (IsNPCActive(out var npc)) {
			...
			if (Projectile.Center.LookForHostileNPC(out NPC player, 1000)) {
				...
				if (++Projectile.ai[0] >= 8) {
					Projectile.ai[0] = 0;
					Projectile.NewProjectile(Projectile.GetSource_FromAI(), Projectile.Center, TowardPlayer.Vector2RotateByRandom(7) * Main.rand.NextFloat(7, 11), ProjectileID.Bullet, Projectile.damage / 3, 1, Projectile.owner);
				}
			}
		}
		else {
...
public class LBL_Musket : BaseGun {
	public override void SetHostileDefaults() {
		Projectile.width = 56;
		...
		if (IsNPCActive(out var npc)) {
			if (Projectile.Center.LookForHostileNPC(out NPC player, 1000)) {
				...
				if (++Projectile.ai[0] >= 40) {
					...
					Projectile.ai[0] = 0;
					Projectile.NewProjectile(Projectile.GetSource_FromAI(), Projectile.Center, TowardPlayer * 15f, ProjectileID.Bullet, Projectile.damage, 1, Projectile.owner);
					for (int i = 0; i < 30; i++) {
...
public class LBL_BoomStick : BaseGun {
	public override void SetHostileDefaults() {
		Projectile.width = 56;
		Projectile.height = 18;
		Projectile.penetrate = -1;
		...
		if (IsNPCActive(out var npc)) {
			if (Projectile.Center.LookForHostileNPC(out NPC player, 1000)) {
				...
				if (Projectile.timeLeft < 30 && Projectile.ai[0] == 0) {
					...
					for (int i = 0; i < 4; i++) {
						Projectile.NewProjectile(Projectile.GetSource_FromAI(), Projectile.Center, TowardPlayer.Vector2RotateByRandom(30) * 12f * Main.rand.NextFloat(.5f, 1f), ProjectileID.Bullet, Projectile.damage, 1, Projectile.owner);
					}
					for (int i = 0; i < 15; i++) {
						int dust = Dust.NewDust(Projectile.Center.PositionOFFSET(TowardPlayer, 10), 0, 0, DustID.Torch);
						Main.dust[dust].noGravity = true;
...
public class LBL_PistolAttackOne : BaseGun {
	public override void SetHostileDefaults() {
		Projectile.width = 32;
	...
	public override void AI() {
		if (IsNPCActive(out var npc)) {
			...
			if (++Projectile.ai[2] >= 12) {
				Projectile.ai[2] = 0;
				Projectile.NewProjectile(Projectile.GetSource_FromAI(), Projectile.Center, TowardTo * 3, ProjectileID.Bullet, Projectile.damage, 1, Projectile.owner);
			}
		}
		else {
			Projectile.Kill();
...
public class LBL_PistolAttackTwo : BaseGun {
	public override void SetHostileDefaults() {
		Projectile.width = 32;
		Projectile.height = 32;
		Projectile.penetrate = -1;
		...
		if (IsNPCActive(out var npc)) {
			if (Projectile.Center.LookForHostileNPC(out NPC player, 1000)) {
				...
				if (++Projectile.ai[0] >= 20) {
					...
					Projectile.NewProjectile(Projectile.GetSource_FromAI(), Projectile.Center, TowardPlayer * 12f, ProjectileID.Bullet, Projectile.damage, 1, Projectile.owner);
...
public class LBL_HostileMinisharkDesperation : BaseGun {
	public override void SetHostileDefaults() {
		Projectile.width = 54;
		Projectile.height = 20;
		Projectile.penetrate = -1;
	...
	public override void AI() {
		if (IsNPCActive(out var npc)) {
			...
			if (Projectile.Center.LookForHostileNPC(out NPC player, 1000)) {
				...
				if (++Projectile.ai[0] >= 8) {
					Projectile.ai[0] = 0;
					Projectile.NewProjectile(Projectile.GetSource_FromAI(), Projectile.Center, TowardPlayer * 7f, ProjectileID.Bullet, Projectile.damage, 1, Projectile.owner);
