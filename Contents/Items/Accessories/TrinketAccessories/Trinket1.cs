...
namespace Roguelike.Contents.Items.Accessories.TrinketAccessories;
public class Trinket_of_Swift_Health : BaseTrinket {
	public override string Texture => ModUtils.GetTheSameTextureAsEntity<Trinket_of_Swift_Health>();
...
public class SwiftSteal_Buff : TrinketBuff {
	...
	public override void OnEnded(Player player) {
		player.GetModPlayer<Trinket_of_Swift_Health_ModPlayer>().Trinket_of_Swift_Health_CoolDown = ModUtils.ToSecond(60);
	}
}
public class Trinket_of_Swift_Health_ModPlayer : ModPlayer {
	...
	public override void PreUpdate() {
		Trinket_of_Swift_Health_DelayBetweenEachHit = ModUtils.CountDown(Trinket_of_Swift_Health_DelayBetweenEachHit);
		if (!Player.HasBuff(ModContent.BuffType<SwiftSteal_Buff>())) {
			Trinket_of_Swift_Health_PointCounter = 0;
			Trinket_of_Swift_Health_CoolDown = ModUtils.CountDown(Trinket_of_Swift_Health_CoolDown);
		}
	}
	...
	private void Trinket_of_Swift_Health_OnHitEffect() {
		...
		if (!Trinket_of_Swift_Health)
			return;
		if (Player.HasBuff(ModContent.BuffType<SwiftSteal_Buff>())) {
			Trinket_of_Swift_Health_DelayBetweenEachHit = ModUtils.ToSecond(2);
