...
namespace Roguelike.Contents.Items.RelicItem.RelicTemplateContent {
	public class WhipTemplate : RelicTemplate {
		public override void SetStaticDefaults() {
			relicType = RelicType.MultiStats;
			RelicTierUPValue = .23f;
		}
		public override PlayerStats StatCondition(Relic relic, Player player) => PlayerStats.SummonDMG;
		...
		public override void Effect(Relic relic, PlayerStatsHandle modplayer, Player player, StatModifier value, PlayerStats stat) {
			if (OutroEffectSystem.Get_Arr_WeaponTag[(int)WeaponTag.Whip].Contains(player.HeldItem.type)) {
