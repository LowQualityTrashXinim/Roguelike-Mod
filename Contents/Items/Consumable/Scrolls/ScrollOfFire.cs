...
internal class ScrollOfFire : ModItem {
	...
	public override bool? UseItem(Player player) {
		if (player.ItemAnimationJustStarted) {
			...
			for (int i = 0; i < 12; i++) {
				var projectile = Projectile.NewProjectileDirect(player.GetSource_ItemUse(Item), player.Center, distance.Vector2DistributeEvenlyPlus(12, 90, i) * 4, ProjectileID.Flamelash, Item.damage, Item.knockBack, player.whoAmI);
