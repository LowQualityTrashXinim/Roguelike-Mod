...
public class AmberBoneSpear : SynergyModItem {
	...
	public override void ModifySynergyShootStats(Player player, PlayerSynergyItemHandle modplayer, ref Vector2 position, ref Vector2 velocity, ref int type, ref int damage, ref float knockback) {
		if (player.altFunctionUse == 2 && !player.HasBuff<AmberBoneThrown_CoolDown>()) {
	...
	public override void SynergyShoot(Player player, PlayerSynergyItemHandle modplayer, EntitySource_ItemUse_WithAmmo source, Vector2 position, Vector2 velocity, int type, int damage, float knockback, out bool CanShootItem) {
		CanShootItem = true;
		if (player.altFunctionUse == 2 && !player.HasBuff<AmberBoneThrown_CoolDown>()) {
			player.AddBuff<AmberBoneThrown_CoolDown>(ModUtils.ToSecond(2) + player.itemAnimationMax);
			return;
		}
		if (++Counter >= 3) {
			Counter = 0;
...
public class AmberBoneThrown_CoolDown : ModBuff {
	public override void SetStaticDefaults() {
		this.BossRushSetDefaultDeBuff();
	}
}
public class AmberBoneProjectile : ModProjectile {
	...
	public override void OnKill(int timeLeft) {
		...
		for (int i = 0; i < 25; i++) {
			int proj = Projectile.NewProjectile(
				...
				Projectile.Center,
				Main.rand.NextVector2CircularEdge(5, 5) * Main.rand.NextFloat(.3f, 1.2f),
				ProjectileID.AmberBolt, Projectile.damage, Projectile.knockBack, Projectile.owner);
