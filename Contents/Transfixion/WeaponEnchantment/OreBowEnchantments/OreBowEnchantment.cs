...
public class CopperBow : ModEnchantment {
	...
	public override void OnHitNPCWithProj(int index, Player player, EnchantmentGlobalItem globalItem, Projectile proj, NPC target, NPC.HitInfo hit, int damageDone) {
		if (proj.arrow && Main.rand.NextBool(10)) {
			target.AddBuff(BuffID.Electrified, ModUtils.ToSecond(Main.rand.Next(1, 5)));
		}
		if (globalItem.Item_Counter1[index] > 0
			|| proj.GetGlobalProjectile<RoguelikeGlobalProjectile>().Source_CustomContextInfo == "WE") {
			return;
		}
		...
		Projectile.NewProjectile(player.GetSource_ItemUse(player.HeldItem, "WE"), pos, (target.Center - pos).SafeNormalize(Vector2.Zero) * 15, ProjectileID.WoodenArrowFriendly, player.GetWeaponDamage(player.HeldItem), 2, player.whoAmI, 0, 0, 9999);
	}
}
public class TinBow : ModEnchantment {
	...
	public override void OnHitNPCWithProj(int index, Player player, EnchantmentGlobalItem globalItem, Projectile proj, NPC target, NPC.HitInfo hit, int damageDone) {
		...
		if (Main.rand.NextBool(8)) {
			Projectile.NewProjectile(player.GetSource_ItemUse(player.HeldItem, "WE"), pos, (target.Center - pos).SafeNormalize(Vector2.Zero) * 15, ModContent.ProjectileType<TinBarProjectile>(), player.GetWeaponDamage(player.HeldItem) + 2, 6, player.whoAmI);
		}
		else {
			Projectile.NewProjectile(player.GetSource_ItemUse(player.HeldItem, "WE"), pos, (target.Center - pos).SafeNormalize(Vector2.Zero) * 15, ProjectileID.WoodenArrowFriendly, player.GetWeaponDamage(player.HeldItem), 2, player.whoAmI, 0, 0, 9999);
		}
...
public class LeadBow : ModEnchantment {
	...
	public override void UpdateHeldItem(int index, Item item, EnchantmentGlobalItem globalItem, Player player) {
		globalItem.Item_Counter1[index] = ModUtils.CountDown(globalItem.Item_Counter1[index]);
		player.ModPlayerStats().DebuffDamage += .08f;
	}
	public override void OnHitNPCWithProj(int index, Player player, EnchantmentGlobalItem globalItem, Projectile proj, NPC target, NPC.HitInfo hit, int damageDone) {
		if (globalItem.Item_Counter1[index] > 0
			|| proj.GetGlobalProjectile<RoguelikeGlobalProjectile>().Source_CustomContextInfo == "WE") {
