...
public class Skill_BoneArrow : ModSkill {
	...
	public override void Update(Player player, SkillHandlePlayer skillplayer) {
		...
		for (int i = 0; i < 16; i++) {
			var vel = Vector2.One.Vector2DistributeEvenlyPlus(16, 360, i).RotatedBy(MathHelper.ToRadians(modplayer.Duration));
			var projlist = skillplayer.NewSkillProjectile(player.GetSource_FromThis(), player.Center, vel, 16f, ProjectileID.BoneArrow, damage, knockback);
...
public class WoodenArrowRain : ModSkill {
	...
	public override void Update(Player player, SkillHandlePlayer skillplayer) {
		...
		var projlist = skillplayer.NewSkillProjectile(player.GetSource_FromThis(), position, Vector2.One * modplayer.Skill_DirectionPlayerFaceBeforeSkillActivation, Main.rand.NextFloat(20, 24), ProjectileID.WoodenArrowFriendly, damage, knockback);
		foreach (var proj in projlist) {
			proj.tileCollide = false;
			proj.timeLeft = 180;
		}
...
public class ChaosArrowRain : ModSkill {
	...
	public override void Update(Player player, SkillHandlePlayer skillplayer) {
		...
		var vel = Vector2.UnitY * Main.rand.NextFloat(20, 24);
		int type = Main.rand.Next(TerrariaArrayID.Arrow);
		if (type == ProjectileID.ShimmerArrow) {
			vel *= -1;
			position.Y += 1000;
		}
		var projlist = skillplayer.NewSkillProjectile(player.GetSource_FromThis(), position, vel, Main.rand.NextFloat(20, 24), type, damage, knockback);
		foreach (var proj in projlist) {
			proj.tileCollide = false;
			proj.timeLeft = 180;
		}
...
public class SpiritBurst : ModSkill {
	...
	public override void OnTrigger(Player player, SkillHandlePlayer skillplayer, int duration, int energy) {
		for (int i = 0; i < 5; i++) {
			...
			var vel = Main.rand.NextVector2CircularEdge(4, 4);
	...
	public override void Update(Player player, SkillHandlePlayer skillplayer) {
		...
		if (Main.rand.NextBool(10) || modplayer.Duration % 15 == 0) {
			...
			float knockback = (int)player.GetTotalKnockback(DamageClass.Magic).ApplyTo(2);
			var vel = Main.rand.NextVector2CircularEdge(4, 4);
			var projlist = skillplayer.NewSkillProjectile(player.GetSource_FromThis(), player.Center, vel, 1, ModContent.ProjectileType<SpiritProjectile>(), damage, knockback);
		}
	}
}
public class Icicle : ModSkill {
	public override void SetDefault() {
		Skill_EnergyRequire = 460;
	...
	public override void Update(Player player, SkillHandlePlayer skillplayer) {
		...
		for (int i = 0; i < 6; i++) {
			...
			var pos = player.Center + Vector2.One.Vector2DistributeEvenlyPlus(6, 360, i).RotatedBy(rotation) * 50;
			for (int l = 0; l < 6; l++) {
...
public class StarFury : ModSkill {
	...
	public override void Update(Player player, SkillHandlePlayer skillplayer) {
		...
		var position = player.Center.Add(Main.rand.Next(-200, 200), 1000);
		var velocity = (Main.MouseWorld - position).SafeNormalize(Vector2.Zero) * 20f;
...
public class MeteorShower : ModSkill {
	...
	public override void Update(Player player, SkillHandlePlayer skillplayer) {
		...
		float knockback = (int)player.GetTotalKnockback(DamageClass.Generic).ApplyTo(10);
		var position = player.Center.Add(Main.rand.Next(-1000, 1000), 1000);
		var velocity = (player.Center - position + Main.rand.NextVector2Circular(500, 500)).SafeNormalize(Vector2.Zero) * 10f;
...
public class BulletStorm : ModSkill {
	public override void SetDefault() {
		Skill_EnergyRequire = 1255;
		Skill_Duration = ModUtils.ToSecond(6);
		Skill_Type = SkillTypeID.Skill_Projectile;
	}
	public override void Update(Player player, SkillHandlePlayer skillplayer) {
		...
		var spawn = player.Center.Add(0, 1000);
...
public class EnergyBolt : ModSkill {
	...
	public override void Update(Player player, SkillHandlePlayer skillplayer) {
		...
		var pos = player.Center + Main.rand.NextVector2Circular(150, 150);
		var toMouse = (Main.MouseWorld - player.Center).SafeNormalize(Vector2.Zero).Vector2RotateByRandom(20);
		var projlist = skillplayer.NewSkillProjectile(player.GetSource_Misc("Skill"), pos, toMouse, 3, ModContent.ProjectileType<EnergyBoltProjectile>(), damage, 2f);
...
public class BulletHell : ModSkill {
	...
	public override void Update(Player player, SkillHandlePlayer skillplayer) {
		...
		if (modplayer.Duration % 100 == 0) {
			for (int i = 0; i < 32; i++) {
				var proj = Projectile.NewProjectileDirect(player.GetSource_FromThis(), player.Center, Vector2.UnitX.Vector2DistributeEvenlyPlus(32, 360, i) * 10, ProjectileID.GoldenBullet, damage, knockback, player.whoAmI);
				proj.tileCollide = false;
				proj.timeLeft = 120;
			}
		}
