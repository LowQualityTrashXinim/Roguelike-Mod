...
namespace Roguelike.Contents.Transfixion.Perks.BlessingPerk;

public class BlessingOfSolar : Perk {
	public override void SetDefaults() {
		textureString = ModUtils.GetTheSameTextureAsEntity<BlessingOfSolar>();
	...
	public override void OnHitNPCWithItem(Player player, Item item, NPC target, NPC.HitInfo hit, int damageDone) {
		...
		if (Main.rand.NextBool(10)) {
			target.AddBuff(ModContent.BuffType<MeltingDefense>(), ModUtils.ToSecond(3.5f));
		}
	}
...
public class BlessingOfVortex : Perk {
	public override void SetDefaults() {
		textureString = ModUtils.GetTheSameTextureAsEntity<BlessingOfVortex>();
		list_category.Add(PerkCategory.Starter);
		CanBeStack = true;
		StackLimit = 3;
	}
	...
	public override void OnHitNPCWithProj(Player player, Projectile proj, NPC target, NPC.HitInfo hit, int damageDone) {
		if (proj.DamageType == DamageClass.Ranged && hit.Crit) {
			player.AddBuff<Buff_VortexBlessing>(ModUtils.ToSecond(2));
		}
	}
}
public class Buff_VortexBlessing : ModBuff {
	public override void Update(Player player, ref int buffIndex) {
		var vortex = player.GetModPlayer<BlessingOfVortexPlayer>();
...
public class BlessingOfNebula : Perk {
	public override void SetDefaults() {
		textureString = ModUtils.GetTheSameTextureAsEntity<BlessingOfNebula>();
		list_category.Add(PerkCategory.Starter);
		CanBeStack = true;
		StackLimit = 3;
	}
...
public class BlessingOfStardust : Perk {
	public override void SetDefaults() {
		textureString = ModUtils.GetTheSameTextureAsEntity<BlessingOfStardust>();
		list_category.Add(PerkCategory.Starter);
		CanBeStack = true;
		StackLimit = 3;
	}
	...
	public override void OnHitNPCWithProj(Player player, Projectile proj, NPC target, NPC.HitInfo hit, int damageDone) {
		if (ProjectileID.Sets.IsAWhip[proj.type]) {
			target.AddBuff(ModContent.BuffType<StarGaze>(), ModUtils.ToSecond(Main.rand.Next(1, 4)));
			if (Main.rand.NextBool(4) && StackAmount(player) >= 3) {
				var spawnPos = target.Center + Main.rand.NextVector2CircularEdge(target.width + 32, target.height + 32);
...
public class StarDustProjectile : ModProjectile {
	...
	public override string Texture => ModUtils.GetVanillaTexture<Item>(ItemID.FragmentStardust);
	public override void SetDefaults() {
	...
	public override void OnHitNPC(NPC target, NPC.HitInfo hit, int damageDone) {
		target.AddBuff<StarRay>(ModUtils.ToSecond(5));
	...
	public override bool PreDraw(ref Color lightColor) {
		...
		var trail = ModContent.Request<Texture2D>(ModTexture.SMALLWHITEBALL).Value;
		var trailOrigin = trail.Size() * .5f;
		var mainOrigin = mainTexture.Size() * .5f;
		for (int k = 0; k < Projectile.oldPos.Length; k++) {
			var drawPos = Projectile.oldPos[k] - Main.screenPosition + mainOrigin;
			var color = new Color(0, 90, 255) * ((Projectile.oldPos.Length - k) / (float)Projectile.oldPos.Length);
			color.A = 0;
		...
		var mainDrawPos = Projectile.position - Main.screenPosition + mainOrigin;
...
public class BlessingOfSynergy : Perk {
	public override void SetDefaults() {
		list_category.Add(PerkCategory.Starter);
		textureString = ModUtils.GetTheSameTextureAsEntity<SynergyEnergy>();
		CanBeStack = true;
		StackLimit = 3;
	}
