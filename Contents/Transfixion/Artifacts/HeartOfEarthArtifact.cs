	...
	class HeartOfEarthPlayer : ModPlayer {
		...
		public override void PostUpdate() {
			if (!Earth || OnHitDelay > 0) {
				OnHitDelay = ModUtils.CountDown(OnHitDelay);
				return;
			}
		...
		public override void UpdateEquip(Player player) {
			...
			foreach (NPC npc in npclist) {
				if (modplayer.synchronize_Counter % 30 == 0) {
					player.StrikeNPCDirect(npc, npc.CalculateHitInfo((player.statLifeMax2 - player.statLife) / 6 * stack, 0, false, ModUtils.DirectionFromPlayerToNPC(player.Center.X, npc.Center.X)));
				}
			}
		}
