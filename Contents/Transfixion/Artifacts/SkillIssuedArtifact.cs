	...
	public class SkillIssuedArtifactPlayer : ModPlayer {
		...
		public override void ResetEffects() {
			...
			if (SkillIssuePlayer) {
				...
				skillissueCallOut = ModUtils.CountDown(skillissueCallOut);
				if (skillissueCallOut == 0) {
					ModUtils.CombatTextRevamp(Player.Hitbox, Main.DiscoColor, "skill issue");
					skillissueCallOut = Main.rand.Next(ModUtils.ToSecond(30), ModUtils.ToMinute(2));
				}
			}
		}
		public override void SyncPlayer(int toWho, int fromWho, bool newPlayer) {
			ModPacket packet = Mod.GetPacket();
			packet.Write((byte)Roguelike.MessageType.SkillIssuePlayer);
