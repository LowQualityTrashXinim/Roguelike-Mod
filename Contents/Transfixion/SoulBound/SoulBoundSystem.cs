...
namespace Roguelike.Contents.Transfixion.SoulBound;
		...
		foreach (var acc in armorItemUpdate) {
			if (acc.IsThisArmorPiece()) {
				...
				if (moditem.SoulBoundSlots.ReachLevelCondition(10)) {
					ModUtils.CombatTextRevamp(Player.Hitbox, SoulBoundLoader.RarityColor(SoulBound.Rarity), $"{SoulBound.DisplayName} level up !");
				}
			}
		}
	}
