...
public class RoguelikeBiomeHandle_ModSystem : ModSystem {
	...
	private void On_Main_DrawBlack(On_Main.orig_DrawBlack orig, Main self, bool force) {
		...
		var vector = Main.drawToScreen ? Vector2.Zero : new Vector2(Main.offScreenRange, Main.offScreenRange);
		var point = new Point(-Main.offScreenRange / 16 + screenOverdrawOffset.X, -Main.offScreenRange / 16 + screenOverdrawOffset.Y);
	...
	private void VanillaReimplementation(Player self, RoguelikeBiomeHandle_ModPlayer modplayer) {
		...
		var point = self.Center.ToTileCoordinates();
		...
		self.ManageSpecialBiomeVisuals("HeatDistortion", Main.UseHeatDistortion && (flag9 || point.Y < Main.worldSurface && self.ZoneDesert && !flag10 && !Main.raining && !Filters.Scene["Sandstorm"].IsActive()));
		if (Main.GraveyardVisualIntensity > 0f) {
			if (!Filters.Scene["Graveyard"].IsActive()) {
				Filters.Scene.Activate("Graveyard", default);
			}
			else {
		...
		if (!Filters.Scene["Sepia"].IsActive() && self.dontStarveShader)
			Filters.Scene.Activate("Sepia", default);
		...
		if (!Filters.Scene["WaterDistortion"].IsActive() && Main.WaveQuality > 0)
			Filters.Scene.Activate("WaterDistortion", default);
		...
		if (Filters.Scene["WaterDistortion"].IsActive()) {
			float num9 = Main.maxTilesX * 0.5f - Math.Abs(point.X - Main.maxTilesX * 0.5f);
			float num10 = 1f;
			...
			num10 += num13;
			float num14 = 1f - MathHelper.Clamp(3f * ((float)(point.Y - Main.worldSurface) / (float)(Main.rockLayer - Main.worldSurface)), 0f, 1f);
			num10 *= num14;
			float num15 = 0.9f - MathHelper.Clamp((Main.maxTilesY - point.Y - 200) / 300f, 0f, 1f) * 0.9f;
			num10 += num15;
		...
		if (flag9) {
			float val = (point.Y - (Main.maxTilesY - 320)) / 120f;
			val = Math.Min(1f, val) * 2f;
			Filters.Scene["HeatDistortion"].GetShader().UseIntensity(val);
		}

		modplayer.shaderOff = Terraria.Utils.Clamp(modplayer.shaderOff + self.behindBackWall.ToDirectionInt() * -0.005f, -0.1f, 1.1f);
		...
		if (Filters.Scene["Sandstorm"].IsActive()) {
			...
			var vector5 = Main.tileColor.ToVector3();
		...
		if (!blizset) {
			try {
				if (flag8) {
					...
					modplayer.shaderdeerclop = MathHelper.Clamp(modplayer.shaderdeerclop + value.ToDirectionInt() * 0.0033333334f, 0f, 1f);
		...
		if (!blizsoundset) {
			try {
				if (flag8) {
					...
					var activeSound = SoundEngine.FindActiveSound(SoundID.BlizzardStrongLoop);
					var activeSound2 = SoundEngine.FindActiveSound(SoundID.BlizzardInsideBuildingLoop);
					if (activeSound == null)
					...
					SoundEngine.TryGetActiveSound(modplayer.insideBlizzardSound, out var sound);
				...
				SoundEngine.TryGetActiveSound(modplayer.strongBlizzardSound, out var activeSound3);
				SoundEngine.TryGetActiveSound(modplayer.insideBlizzardSound, out var activeSound4);
				if (modplayer.strongblizzVol > 0f) {
					if (activeSound3 == null) {
						modplayer.strongBlizzardSound = SoundEngine.PlaySound(SoundID.BlizzardStrongLoop);
						SoundEngine.TryGetActiveSound(modplayer.strongBlizzardSound, out var soundResult);
					...
					if (activeSound4 == null) {
						modplayer.insideBlizzardSound = SoundEngine.PlaySound(SoundID.BlizzardInsideBuildingLoop);
						SoundEngine.TryGetActiveSound(modplayer.insideBlizzardSound, out var soundResult);
		...
		if (!self.dead) {
			var point2 = self.Center.ToTileCoordinates();
			// Extra patch context.
			...
			if (self.position.Y / 16f > Main.UnderworldLayer)
	...
	private void On_Player_UpdateBiomes(On_Player.orig_UpdateBiomes orig, Player self) {
		...
		var tileSafely = Framing.GetTileSafely(self.Center);
		if (tileSafely != null)
			self.behindBackWall = tileSafely.WallType > WallID.None;
		//underworldHeight.SetValue(null, Main.maxTilesY - 200);
		var modplayer = self.GetModPlayer<RoguelikeBiomeHandle_ModPlayer>();
		var toTile = self.Center.ToTileCoordinates();
		Main.worldSurface = 0;
		Main.rockLayer = 0;
...
internal class RoguelikeBiomeHandle_GlobalNPC : GlobalNPC {
	public override void EditSpawnPool(IDictionary<int, float> pool, NPCSpawnInfo spawnInfo) {
		...
		var modplayer = spawnInfo.Player.GetModPlayer<RoguelikeBiomeHandle_ModPlayer>();
