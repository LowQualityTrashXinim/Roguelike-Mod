...
new Point(167,92),];
	...
	public override void PostUpdateEverything() {
		...
		if (player.Center.IsCloseToPosition(Pos_Pyramid.Center().ToWorldCoordinates(), 1500)) {
			if (!IsWithinRange) {
				var points = PyramidModObjectSpawningPoint();
				foreach (var point in points) {
					var worldPos = (Pos_Pyramid.Location + point).ToWorldCoordinates();
...
public class Sealed_SandElemental : NPCSealedObject {
	...
	public override void Inner_AI() {
		...
		if (BreakSeal) {
			counter++;
			var pos = Center + Main.rand.NextVector2Circular(50, 50);
			var dust = Dust.NewDustDirect(pos, 0, 0, DustID.Sand);
	...
	public override void OnKill() {
		if (BreakSeal) {
			var npc = NPC.NewNPCDirect(new EntitySource_WorldGen(), (int)position.X, (int)position.Y, NPCTypeToFollow);
			BreakSeal = false;
		}
	}
}
