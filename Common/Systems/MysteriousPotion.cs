	...
	public class MysteriousPotionBuff : ModBuff {
		...
		public static void StatsCalculation(int addpoint, Player player, MysteriousPotionPlayer modplayer) {
			...
			for (int i = 0; i < modplayer.Stats.Count; i++) {
				...
				ModUtils.CombatTextRevamp(player.Hitbox, textcolor, StatNumberAsText(modplayer, i), i * 20, 180);
			}
		}
		public static string StatNumberAsText(MysteriousPotionPlayer modplayer, int index) {
			string value = "";
			if (modplayer.StatsMulti[index] > 0) value = "+";
			if (ModUtils.DoesStatsRequiredWholeNumber(modplayer.Stats[index])) return value + $"{modplayer.ToStatsNumInt(modplayer.Stats[index], modplayer.StatsMulti[index])} {modplayer.Stats[index]}";
		...
		public override void ModifyBuffText(ref string buffName, ref string tip, ref int rare) {
			...
			tip = "";
			for (int i = 0; i < modplayer.Stats.Count; i++) {
				if (ModUtils.DoesStatsRequiredWholeNumber(modplayer.Stats[i])) {
