...
public class UniversalModPlayer : ModPlayer {
	...
	public override void OnEnterWorld() {
		...
		if (UniqueID == string.Empty) {
			ModUtils.CombatTextRevamp(Player.Hitbox, Color.Yellow, "Detected : using a unrecognized player character ");
			UniqueID = RoguelikeData.Run_Amount + ModUtils.JumboString(Main.rand, Player.name + Main.worldName);
		}
		else {
...
public class DefaultUI : UIState {
	...
	public override void OnInitialize() {
		...
		Append(staticticUI);
		colorChanging1 = new(new() { new Color(243, 133, 71), Color.White });
		colorChanging2 = new(new() { Color.White, new Color(243, 133, 71) }, .5f);
	...
	private void CreateEnergyBar() {
		...
		energyBar = new Roguelike_ProgressUIBar(null, new Color(243, 133, 71), Color.White, "0/0", .8f);
	...
	public void TurnOnEndOfDemoMessage() {
		...
		EndOfDemoPanelClose.OnUpdate += EndOfDemoPanelClose_OnUpdate;
		endofdemo_Main.Append(EndOfDemoPanelClose);
	}

	private void EndOfDemoPanelClose_OnUpdate(UIElement affectedElement) {
...
public class WeaponDPSimage : UIImageButton {
	...
	public WeaponDPSimage(int itemtype, int dps) : base(ModContent.Request<Texture2D>(ModTexture.ACCESSORIESSLOT)) {
		this.itemtype = itemtype;
		this.dps = dps;
	}
	public override void Update(GameTime gameTime) {
	...
	public override void Draw(SpriteBatch spriteBatch) {
		...
		Texture2D originaltexture = ModContent.Request<Texture2D>(ModTexture.ACCESSORIESSLOT).Value;
		Texture2D texture = TextureAssets.Item[itemtype].Value;
		Vector2 origin = texture.Size() * .5f;
		Vector2 drawpos = GetDimensions().Position() + originaltexture.Size() * .5f;
