...
public abstract class ModAchievement {
	...
	public virtual string Texture => ModTexture.ACCESSORIESSLOT;
	...
	public string DisplayName => Language.GetTextValue($"Mods.Roguelike.Achievement.{Name}.DisplayName");
	public string Description => Language.GetTextValue($"Mods.Roguelike.Achievement.{Name}.Description");
	public string ConditionTip => Language.GetTextValue($"Mods.Roguelike.Achievement.{Name}.ConditionTip");
	public string ConditionTipAfterAchieve => Language.GetTextValue($"Mods.Roguelike.Achievement.{Name}.ConditionTipAfterAchieve");
	public string Reward => Language.GetTextValue($"Mods.Roguelike.Achievement.{Name}.Reward");
...
public class AchievementUI : UIState {
	...
	public override void OnInitialize() {
		asset = ModContent.Request<Texture2D>(ModTexture.ACCESSORIESSLOT);
		hash_tag = new();
		lib_achievement = new();
...
public class PageImage : UIImage {
	...
	public PageImage(Asset<Texture2D> texture) : base(texture) {
		unselected = ModContent.Request<Texture2D>(ModTexture.Page_StateUnselected);
		selected = ModContent.Request<Texture2D>(ModTexture.Page_StateSelected);
	}
	public override void Draw(SpriteBatch spriteBatch) {
		if (!CustomDraw) {
			...
			Vector2 drawpos = GetInnerDimensions().Position() + originalorigin * .5f + originalorigin * .25f;
			Vector2 origin;
			if (!toggled) {
...
public class AchievementHeaderPreview : UIImageButton {
	...
	public AchievementHeaderPreview(Asset<Texture2D> texture, string achievementName) : base(texture) {
		...
		achieved = ModContent.Request<Texture2D>(ModTexture.CommonTextureStringPattern + "UI/complete");
	...
	public override void Draw(SpriteBatch spriteBatch) {
		...
		if (checkAchievement) {
			...
			if (achievement.SpecialDraw) {
				...
				if (!achievement.Achieved) {
					...
					Vector2 drawpos2 = GetDimensions().Position() + texture.Size() * .5f;
			...
			texturestring = ModTexture.MissingTexture_Default;
		}
		...
		Vector2 drawpos = GetDimensions().Position() + texture.Size() * .5f;
