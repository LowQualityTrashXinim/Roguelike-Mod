...
public class StructureUI : UIState {
	...
	Asset<Texture2D> defaultlookingassbtn = ModContent.Request<Texture2D>(ModTexture.ACCESSORIESSLOT);
	...
	public override void Draw(SpriteBatch spriteBatch) {
		...
		if (CurrentUI_State == StructureUI_State.Selecting) {
			...
			if (secondPoint) {
				...
				float scale1 = Vector2.Distance(Main.MouseWorld, point1.ToVector2() * 16) < 32 ? 1.5f : 1f;
				spriteBatch.Draw(tex, point1.ToVector2() * 16 - Main.screenPosition, tex.Frame(), Color.Cyan * scale1, 0, tex.Frame().Size() / 2, scale1, 0, 0);

				float scale2 = Vector2.Distance(Main.MouseWorld, point2.ToVector2() * 16) < 32 ? 1.5f : 1f;
				spriteBatch.Draw(tex, point2.ToVector2() * 16 - Main.screenPosition, tex.Frame(), Color.Red * scale2, 0, tex.Frame().Size() / 2, scale2, 0, 0);
