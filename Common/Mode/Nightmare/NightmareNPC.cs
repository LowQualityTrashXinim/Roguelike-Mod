...
namespace Roguelike.Common.Mode.Nightmare
{
	internal class NightmareNPC : GlobalNPC {
		...
		public override void PostAI(NPC npc) {
			...
			if (npc.type == NPCID.Spazmatism) {
				...
				if (!ModUtils.CompareSquareFloatValue(npc.Center, Main.player[npc.target].Center, 2000 * 2000)) {
		...
		private void KingSlimeOnAir(NPC npc) {
			if (aiTimer % 10 == 0) {
				...
				if (Main.player[npc.target].Center.Y > npc.Center.Y) {
					towardPlayer = towardPlayer.RotatedBy(MathHelper.ToRadians(-ModUtils.DirectionFromPlayerToNPC(Main.player[npc.target].Center.X, npc.Center.X) * 15));
				}
