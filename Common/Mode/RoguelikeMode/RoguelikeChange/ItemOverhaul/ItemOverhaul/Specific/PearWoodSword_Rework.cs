...
public class Roguelike_PearlWoodSword : GlobalItem {
	...
	public override void HoldItem(Item item, Player player) {
		if (player.itemAnimation == player.itemAnimationMax && player.ItemAnimationActive) {
			...
			Roguelike_PearlWoodSword_ModPlayer modplayer = player.GetModPlayer<Roguelike_PearlWoodSword_ModPlayer>();
			if (++modplayer.swingCount >= 3) {
				...
				for (int i = 0; i < 6; i++) {
					...
					Projectile projectile = Projectile.NewProjectileDirect(player.GetSource_ItemUse(item), pos, vel, ModContent.ProjectileType<pearlSwordProj>(), damage, 1, player.whoAmI);
...
public class Roguelike_PearlWoodSword_ModPlayer : ModPlayer {
	...
	public override void UpdateEquips() {
		if (duration > 0 && duration % 5 == 0) {
			...
			Vector2 pos = lastHitLocation + Main.rand.NextVector2CircularEdge(250, 250) * Main.rand.NextFloat(1, 3);
			var vel = (Main.MouseWorld + Main.rand.NextVector2Circular(20, 20) - pos).SafeNormalize(Vector2.Zero);
			Projectile projectile = Projectile.NewProjectileDirect(Player.GetSource_ItemUse(Player.HeldItem), pos, vel, ModContent.ProjectileType<pearlSwordProj>(), damage, 1, Player.whoAmI);
