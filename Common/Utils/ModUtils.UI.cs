...
namespace Roguelike.Common.Utils {
	...
	public class Roguelike_ProgressUIBar : UIElement {
		...
		public Roguelike_ProgressUIBar(Asset<Texture2D> bartexture, Color starterColor, Color endColor, string textstring, float textscale = 1, bool isLarge = false) {
			if (bartexture == null) {
				barFrame = new UIImage(ModContent.Request<Texture2D>(ModTexture.EXAMPLEUI)); // Frame of our resource bar
			}
			else {
	...
	public class WeaponProgress {
		...
		/// <summary>
		/// Setting this true mean that this weapon doesn't contain timing base<br/>
		/// Otherwise false mean there is a timing elements
		/// </summary>
